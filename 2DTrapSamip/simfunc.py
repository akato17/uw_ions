...
def leap_frog2(N, init_cdn, fm):
    ...
    # Same as dt in the Newton 3 function
    accf = 1 / m * (coulomb_vect(pos,N) + field_vect(pos, N, VOLT0and4) + FHYP_pseudo(pos,N) + laser.laser_vect(vel,N))
    ...
    while it < iterations:
        ...
        #accf = 1 / m * (coulomb_vect(pos,N) + 10* field_vect(pos, N, VOLT0and4) + FHYP_pseudo(pos,N) + laser.laser_vect(vel,N))
        #accf = 1 / m * (coulomb_vect(pos,N) + 10* field_vect(pos, N, VOLT0and4) + FHYP_pseudo(pos,N) + laser.discrete_laser(vel,N))
        #accf = 1 / m * (coulomb_vect(pos,N) + 10* field_vect(pos, N, VOLT0and4) + FHYP_pseudo(pos,N))# for v kicks
        #accf= (1/m)*(coulomb_vect(pos,N) + 10 * field_vect(pos, N, VOLT0and4) + 14* field_vect(pos, N, VOLT8and9) + 1000 * field_vect(pos, N, VOLT8and9 * np.cos(omega*t)) + laser.laser_vect(vel,N))
        #accf= 1 / m *(coulomb_vect(pos,N) + 10 * field_vect(pos, N, VOLT0and4) + 14* field_vect(pos, N, VOLT8and9) + 1000 * field_vect(pos, N, VOLT8and9 * np.cos(omega*t)) + damp(vel,N,t,twindow))
        accf= 1/ m *(coulomb_vect(pos,N) + FHYP_Trap_Frequency(252000*2*np.pi,173250*2*np.pi,764000*2*np.pi,pos,N) + damp(vel,N,t,twindow))
...
@njit
####returns trap force based on input trap frequency-- mostly to confirm if crystals are 2D or not
