...
w=int(n_points/2)
####select your data
Vdata=A[0][200-w:200+w+1,200-w:200+w+1,200-w:200+w+1]
###flatten to 1d array
Vfit=Vdata.ravel()
####also select some dc data
Vdata2=A[1][200-w:200+w+1,200-w:200+w+1,200-w:200+w+1]
####flatten to 1d array
Vfit2=Vdata2.ravel()
...
cords=A[4][200-w:200+w+1,200-w:200+w+1,200-w:200+w+1].reshape((len(Vfit),3))-200
######make array of alll coordinates centered at 0, for reproducing entire data set
allcords=A[4].reshape((len(A[0])**3),3)-200
#####guesses for RF fit
guess = (-1e-4,-1e-4,1e-4,-1e-7,-1e-7,1e-7,1000)
######guesses for DC fit
guess2 = (-1e-4,-1e-4,1e-4,-1e-3,-1e-3,1e-3,10)
##########convert coordinates to cartesian coordinates
cart=(A[4]-200)*(.01/400)*1e6
####perform the fits
params, pcov = optimize.curve_fit(fcurve, cords, Vfit, guess)
params2, pcov2 = optimize.curve_fit(fcurve, cords, Vfit2, guess2)
...
######make fave RF potential

B=fake_V(cords,params)
###make fake DC potential
C=fake_V(cords,params2)
###make full sized rf potential
D=fake_V(allcords,params)
###make full sized dc potential
E=fake_V(allcords,params2)

####plot rf potential and fitted rf potential for comparison, x axis as micrometers
# plt.plot(np.array(cart[200,200,:,2]),D[200,200,:])
# plt.plot(np.array(cart[200,200,:,2]),A[0][200,200,:])