...
def curve_fit(V0RF, V0DC, cord):
    ...
    ######make array of all coordinates centered at 0, for reproducing entire data set
    ncords=(cord.reshape((size**3),3)-h)
    #####guesses for RF fit
    guessRF = (-1e-4,-1e-4,1e-4,-1e-7,-1e-7,1e-7,1000)
    ######guesses for DC fit
    guessDC = (-1e-4,-1e-4,1e-4,-1e-3,-1e-3,1e-3,30)
    ####perform the fits
    ...
    return paramsRF, paramsDC, ncords
...
# Determine the magnitude of E field within the grid
def Efield_solve(VRF):
    
    # Calculate the components from the gradient of V_RF
    Ex=np.zeros((size,size,size))
    Ey=np.zeros((size,size,size))
    Ez=np.zeros((size,size,size))
    Ex,Ey,Ez = np.gradient(VRF,grid/size)
    
    # Initialize the output
    Emag=np.zeros((size,size,size))
    
    # Take 3D mod of the vector
