...
def pseudopotential(V,omega):
    w=np.gradient(V)
    mag=w[0]**2+w[1]**2+w[2]**2
    return e**2/(4*m*omega**2)*mag
...
def fcurve_Octupole(data,k1,k2,k3,k4,k5,k6,k7,k8,k9,k10,k11,k12,k13,k14,k15,k16):

      x=data[:,0]
      y=data[:,1]
      z=data[:,2]
      terms=np.array([k1*x**4,k2*x**3*y,k3*x**3*z,k4*x**2*y**2,k5*x**2*z**2,k6*x**2*y*z,k7*x*y**3,k8*x*z**3,\
             k9*x*y**2*z,k10*x*y*z**2,k11*y**4,k12*y**3*z,k13*y**2*z**2,k14*y*z**3,k15*z**4,k16],dtype=object)
      return np.sum(terms)#+h*x+i*y+j*z
  
def fcurve_Power_Series(data,k1,k2,k3,k4,k5,k6,k7,k8,k9,k10,k11,k12,k13,k14,k15,k16,j1,j2,j3,j4,j5,j6,j7,j8,j9,j10,l1,l2,l3,l4,l5,l6,m1,m2,m3):
#################instead of enforcing 4th order lets see what wants to fit best
      x=data[:,0]
      y=data[:,1]
      z=data[:,2]
      terms4=np.array([k1*x**4,k2*x**3*y,k3*x**3*z,k4*x**2*y**2,k5*x**2*z**2,k6*x**2*y*z,k7*x*y**3,k8*x*z**3,\
              k9*x*y**2*z,k10*x*y*z**2,k11*y**4,k12*y**3*z,k13*y**2*z**2,k14*y*z**3,k15*z**4,k16],dtype=object)
      terms3=np.array([j1*x**3,j2*x**2*y,j3*x**2*z,j4*x*y**2,j5*x*z**2,j6*x*y*z,j7*y**3,j8*y**2*z,j9*z**3,j10*z**2*y],dtype=object)
      terms2=np.array([l1*x**2,l2*x*y,l3*x*z,l4*y**2,l5*y*z,l6*z**2],dtype=object)
      terms1=np.array([m1*x,m2*y,m3*z],dtype=object)

      return np.sum(terms4)+np.sum(terms3)+np.sum(terms2)+np.sum(terms1)#+h*x+i*y+j*z
  
def fcurve_test(data,k1,k2,k3,k7):
#################instead of enforcing 4th order lets see what wants to fit best
      x=data[:,0]
      y=data[:,1]
      z=data[:,2]
      terms=np.array([k1*x**4,k2*y**4,k3*z**2,k7],dtype=object)
    

      return np.sum(terms)

def curve_fit_Octupole(V0RF, V0DC, cord, VRF, VDC):
    ####choose number of points to fit at center of trap (should be odd if centered at 0)
    w = int(n_points / 2)
    ####select your data
    h = int((size - 1) / 2)
    ...
    cords = cord[h-w:h+w+1,h-w:h+w+1,h-w:h+w+1].reshape((len(VfitRF),3))-h
    #####guesses for RF fit
    guessRF = (-1e-4,-1e-4,1e-4,-1e-7,-1e-7,1e-7,-1e-7,-1e-7,1e-7,-1e-7,-1e-7,1e-7,-1e-7,-1e-7,1e-7,VRF)
    ######guesses for DC fit
    guessDC = (-1e-4,-1e-4,1e-4,-1e-3,-1e-3,1e-3,-1e-7,-1e-7,1e-7,-1e-7,-1e-7,1e-7,-1e-7,-1e-7,1e-7,VDC)
    ####perform the fits
    paramsRF_Octupole, pcovRF = optimize.curve_fit(fcurve_Octupole, cords, VfitRF, guessRF)
    paramsDC_Octupole, pcovDC = optimize.curve_fit(fcurve_Octupole, cords, VfitDC, guessDC)
    return paramsRF_Octupole, paramsDC_Octupole,cords

def curve_fit_test(V0RF, V0DC, cord, VRF, VDC):
    ####choose number of points to fit at center of trap (should be odd if centered at 0)
    w = int(n_points / 2)
    ####select your data
    h = int((size - 1) / 2)
    ...
    cords = cord[h-w:h+w+1,h-w:h+w+1,h-w:h+w+1].reshape((len(VfitRF),3))-h
    #####guesses for RF fit
    guessRF = (-1e-7,-1e-7,1e-4,VDC)
    ######guesses for DC fit
    guessDC = (-1e-4,-1e-4,1e-4,VDC)
    ####perform the fits
    paramsRF_Octupole, pcovRF = optimize.curve_fit(fcurve_test, cords, VfitRF, guessRF)
    paramsDC_Octupole, pcovDC = optimize.curve_fit(fcurve_test, cords, VfitDC, guessDC)
    return paramsRF_Octupole, paramsDC_Octupole,cords

def curve_fit_Power_Series(V0RF, V0DC, cord, VRF, VDC):
    ####choose number of points to fit at center of trap (should be odd if centered at 0)
    w = int(n_points / 2)
    ####select your data
    h = int((size - 1) / 2)
    ...
    cords = cord[h-w:h+w+1,h-w:h+w+1,h-w:h+w+1].reshape((len(VfitRF),3))-h
    #####guesses for RF fit
    guessRF = (-1e-8,-1e-14,1e-14,1e-8,-1e-14,1e-14,-1e-14,-1e-10,1e-14,-1e-10,-1e-8,1e-10,-1e-10,-1e-10,1e-10,1e-24\
               ,1e-14,1e-14,1e-14,1e-14,1e-14,1e-14,1e-14,1e-14,1e-14,1e-14,-1e-4,-1e-8,1e-8,-1e-4,1e-8,1e-2,0,0,0)
    ######guesses for DC fit
    guessDC = guessRF
    ####perform the fits
    paramsRF_Octupole, pcovRF = optimize.curve_fit(fcurve_Power_Series, cords, VfitRF, guessRF)
    paramsDC_Octupole, pcovDC = optimize.curve_fit(fcurve_Power_Series, cords, VfitDC, guessDC)
    return paramsRF_Octupole, paramsDC_Octupole,cords

def fake_V_Octupole(cord,params):
    w = int(n_points / 2)
    h = int((size - 1) / 2)
    ...
    for i in range(0,len(cord)):
        ...
        # print(x)
        terms=np.array([k1*x**4,k2*x**3*y,k3*x**3*z,k4*x**2*y**2,k5*x**2*z**2,k6*x**2*y*z,k7*x*y**3,k8*x*z**3,\
             k9*x*y**2*z,k10*x*y*z**2,k11*y**4,k12*y**3*z,k13*y**2*z**2,k14*y*z**3,k15*z**4,k16])
        A=np.sum(terms)
        # print(A)
        fakeV[i]=A#+h*x+i*y+|j*z
    L=int(len(cord)**(1/3))+1
    return fakeV.reshape((L,L,L))
...
scale=200/.005
scale_array=np.array([scale**4,scale**4,scale**4,scale**4,scale**4,scale**4,scale**4,scale**4,scale**4,scale**4,scale**4,scale**4,scale**4,scale**4,scale**4,1,scale**3,scale**3,scale**3,scale**3,scale**3,scale**3,scale**3,scale**3,scale**3,scale**3,scale**2,scale**2,scale**2,scale**2,scale**2,scale**2,scale,scale,scale])